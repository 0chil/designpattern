# 데코레이터 패턴
복습 22/9/5, 벌써 생각이 달라진 부분들이 좀 있고, 당시에는 생각하지 못했던 것들이 보인다.
...
또 다른 데코레이터를 만들자!  
- 이건 더 복잡해질 것 같다...
- 그냥 장식물의 가격이 사이즈에 따라 달라지므로 장식물에 조건을 다는게 제일 인듯!

#### 책에서 말하는 정답
![img.png](image/img_11.png)
🧠 음..? 좀 이상한데..?  
베이스가 되는 음료의 사이즈가 상관있으니 맞긴 하다.  
달리 이런 요금을 명시할 곳도 없긴 하다.  
장단점을 한번 따져보자
- 장점
  - `Size`에 따라 일일이 `Decorator`를 만들지 않아도 된다.
  - `Size`에 따라 다른 요금을 명시할 수 있다.
- 단점
  - 코드가 ~~(많이)~~ 복잡해졌다. 근데 이건 처음부터 `Size`를 저런식으로 추가했기 때문이리라.
  - 개인적으로 마음에 들지 않는 디자인. (특히 `Beverage::getSize`의 오버라이딩) 
    - 근데 딱히 대안이 없어보임.
    - 나는 `Size`는 생성자를 통해 받도록 했다.
  - ~~오버라이딩 파티...~~ -> 이건 `abstract` 메소드라서 괜찮아!

🧠 장단점을 따져보니 딱히 대안이 없다. 차선책인듯. ㅇㅋ.  
시간이 지나고 실제로 사용해 봐야 알겠다.

#### 부가적인 생각
커피는 사이즈에 따라서도 가격이 달라진다.
커피 구현체의 `cost()`에도 `Size`에 따라 달라지는 요금을 적용해야겠다.

### 쓰임새
#### 인터넷에서 찾아 봄
[지하철 미션](https://giron.tistory.com/114)에 사용하셨다.  
아직 정확히 판단하기는 힘들지만 **팩토리나 빌더 패턴과 사용**하면 좋을 것 같다.

#### `java.io` 패키지의 `InputStream`
![img.png](image/img_12.png)

![img.png](image/img_13.png)
🧠 (아까 [Q. 이런 코드를 근본적으로 막을 수는 없나?](Q.+이런+코드를+근본적으로+막을+수는+없나)에서 사용한 디자인이 나쁘지는 않았나 보다)
